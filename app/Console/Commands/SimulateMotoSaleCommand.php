...
class SimulateMotoSaleCommand extends Command
{
    protected $signature = 'pos:moto-sale';
    protected $description = 'Simula una venta de repuestos de motos con IVA e IGTF en Venezuela';
    ...
    public function handle()
    {
        ...
        $this->line('<fg=bright-red;options=bold> 🏍️ MEGA TIENDA DE REPUESTOS (BERA/EMPIRE/MD) - VENEZUELA 2026 </>');
        $this->line('<fg=bright-cyan>===============================================================</>');
        $this->line(' <fg=gray>Simulador de Compra de Repuestos (Todos Gravables 16% IVA)</>');
        $this->line('<fg=bright-cyan>===============================================================</>');
        $this->line('');
        ...
        $this->info('1️⃣ CONFIGURANDO SESIÓN DE CAJA (REPUESTOS)...');
        $caja = Caja::first();
        ...
        $this->info("\n2️⃣ PROCESANDO REPUESTOS DE MOTOCICLETA...");
        ...
        // Todos estos repuestos son gravables al 16% en Venezuela
        $agregarItem('Caucho Trasero 18" Bera SBR', 1, 16);
        $agregarItem('Tripa 18" Reforzada', 1, 16);
        $agregarItem('Batería Seca 12V 7Ah', 1, 16);
        $agregarItem('Kit de Arrastre Bera/Horse', 1, 16);
        $agregarItem('Aceite Mineral Motul 20W50 4T 1L', 2, 16);
        $agregarItem('Bujía NGK Original', 1, 16);
        $agregarItem('Pastillas de Freno Delantero', 1, 16);

        $venta->refresh();
        $this->line("   <fg=bright-yellow;options=bold>=> Subtotal Repuestos (con IVA): $" . number_format($venta->total, 2) . "</>");

        $this->info("\n3️⃣ PROCESANDO PAGOS (DIVISAS + BOLÍVARES)...");
        
        $efectivoUSD = MetodoPago::where('nombre', 'Efectivo USD')->first();
        $pagoMovil = MetodoPago::where('nombre', 'Pago Móvil VES')->first();

        // Pago 1: $50 en Efectivo USD (IGTF 3% = $1.50 sobre los $50 en divisa)
        $montoEfectivo = 50.00;
        $this->posService->registrarPagoConIgtf($venta, $efectivoUSD, $montoEfectivo, 1.0);
        $igtfCalculado = round($montoEfectivo * 0.03, 2);
        $this->line("   💳 Efectivo USD: $" . $montoEfectivo . " <fg=magenta>(+ IGTF 3% = $$igtfCalculado)</>");
        ...
        $this->info("\n4️⃣ IMPRIMIENDO FACTURA FISCAL DEL TALLER...");
        $venta = $this->posService->procesarCobroFactura($venta);
        $venta->refresh();

        // Calcular bases manuales
        $exento = 0;
        $baseImponible = 0;
        ...
        $this->line("<fg=black;bg=white;options=bold>              INVERSIONES MOTO REPUESTOS 2026 C.A.              </>");
        $this->line("<fg=black;bg=white>                     RIF: J-98765432-1                          </>");
        $this->line("<fg=black;bg=white>                                                                </>");
        ...
        $this->line("<fg=black;bg=white>                                                                </>");
        $this->line('');
        $this->info("✨ ¡Venta de Repuestos (IVA + IGTF) Completada con Éxito!");
    }
}
